...
class EstatisticasView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        registros = RegistroDiario.objects.filter(meta__usuario=usuario)
        ...
        if periodo == "semana":
            ...
            registros = registros.filter(data__year=hoje.year)

        total_registros = registros.count()
        total_check = registros.filter(status_conclusao="check").count()
        total_falha = registros.filter(status_conclusao="falha").count()
        total_branco = registros.filter(status_conclusao="branco").count()

        perfil, _ = PerfilGamificacao.objects.get_or_create(usuario=usuario)

        perfil.verificar_quebra_de_ofensiva()
